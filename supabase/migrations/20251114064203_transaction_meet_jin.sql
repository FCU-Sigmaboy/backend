...
END IF;
  ...
  -- 5. 獲取物品價格和碳足跡
SELECT price, carbon_value INTO v_item FROM public.items WHERE id = v_transaction.item_id;
v_price := v_item.price;
v_carbon_value := v_item.carbon_value;
...
VALUES (v_receiver_id, -v_price, 'transaction_expense', p_transaction_id, '索取物品：' || v_item.title);
...
-- 10. (日誌) 新增賣家點數收入紀錄
INSERT INTO public.point_logs (user_id, amount, type, transaction_id, description)
VALUES (v_transaction.giver_id, v_price, 'transaction_income', p_transaction_id, '售出物品：' || v_item.title);
