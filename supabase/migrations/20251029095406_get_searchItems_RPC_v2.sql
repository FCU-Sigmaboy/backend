...
IF LOWER(p_sort_direction) = 'asc' THEN
        IF LOWER(p_sort_by) = 'distance' AND v_user_primary_location IS NOT NULL THEN
            RETURN QUERY
                WITH items_with_distance AS (SELECT i.*,
                                                    u.nickname,
                                                    u.profile_picture_url,
                                                    sc.main_category_id,
                                                    l.formatted_address,
                                                    l.coordinates AS item_coordinates, -- *** 獲取物品座標 ***
                                                    ROUND(
                                                            (ST_Distance(l.coordinates, v_user_primary_location) / 1000.0)::numeric,
                                                            3)    AS distance_km       -- *** 使用 l.coordinates 計算 ***
                                             FROM public.items i
...
SELECT iwd.id,
       ...
       json_build_object('id', iwd.user_id, 'nickname', iwd.nickname, 'profile_picture_url',
                         iwd.profile_picture_url)
FROM items_with_distance iwd
-- *** 修正 JOIN 子查詢 ***
         LEFT JOIN (SELECT f.item_id, count(*) AS fav_count
                    FROM public.favorites f
                    GROUP BY f.item_id) fav ON iwd.id = fav.item_id
WHERE (p_distance_range_km IS NULL OR
       (v_user_primary_location IS NOT NULL AND iwd.distance_km <= p_distance_range_km))
  AND (p_main_category_id IS NULL OR iwd.main_category_id = p_main_category_id)
  AND (p_sub_category_id IS NULL OR iwd.sub_category_id = p_sub_category_id)
  AND (p_user_id IS NULL OR iwd.user_id = p_user_id)
  AND (p_keyword IS NULL OR (iwd.title ILIKE '%' || p_keyword || '%' OR iwd.tags @> ARRAY [p_keyword]))
ORDER BY iwd.distance_km ASC NULLS LAST, iwd.created_at DESC
...
WHERE (p_distance_range_km IS NULL OR
  ...
  AND (p_user_id IS NULL OR iwd.user_id = p_user_id)
  AND (p_keyword IS NULL OR (iwd.title ILIKE '%' || p_keyword || '%' OR iwd.tags @> ARRAY [p_keyword]))
ORDER BY iwd.price ASC, iwd.created_at DESC
LIMIT p_size OFFSET v_offset;
...
WHERE (p_distance_range_km IS NULL OR
  ...
  AND (p_user_id IS NULL OR iwd.user_id = p_user_id)
  AND (p_keyword IS NULL OR (iwd.title ILIKE '%' || p_keyword || '%' OR iwd.tags @> ARRAY [p_keyword]))
ORDER BY iwd.distance_km DESC NULLS LAST, iwd.created_at DESC
LIMIT p_size OFFSET v_offset;
...
WHERE (p_distance_range_km IS NULL OR
  ...
  AND (p_user_id IS NULL OR iwd.user_id = p_user_id)
  AND (p_keyword IS NULL OR (iwd.title ILIKE '%' || p_keyword || '%' OR iwd.tags @> ARRAY [p_keyword]))
ORDER BY iwd.price DESC, iwd.created_at DESC
LIMIT p_size OFFSET v_offset;
