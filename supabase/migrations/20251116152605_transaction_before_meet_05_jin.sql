...
-- ### *** 已更新：新增 reviewed_at 欄位 ***
-- ####################################################################

-- 步驟 1: (必須) 刪除舊函式，因為我們要變更 "回傳類型"
DROP FUNCTION IF EXISTS public.get_my_transactions_by_status(TEXT, TEXT);
DROP FUNCTION IF EXISTS public.get_my_transactions_by_status(VARCHAR, TEXT);

-- 步驟 2: 建立新函式 (已新增 reviewed_at 欄位)
CREATE OR REPLACE FUNCTION public.get_my_transactions_by_status(
    p_status TEXT, -- 參數類型 TEXT (因為欄位是 VARCHAR)
    ...
    item_title VARCHAR(50),
              ...
              -- *** 新增的欄位 ***
              reviewed_at TIMESTAMPTZ -- "我" 評價這筆交易的時間 (如果已評)
              )
...
SELECT
    ...
    u.id, u.nickname,
    -- *** 新增：查找 "我" (v_current_uid) 寫的評價 ***
    (SELECT r.created_at
     FROM public.ratings r
     WHERE r.transaction_id = t.id
       AND r.reviewer_id = v_current_uid
    ) AS reviewed_at
