-- 整合 use_primary_location + approximate_location)
...
-- *** 最終回傳結構 ***
    ...
    title VARCHAR(50),
              ...
              price INT,
              distance_km NUMERIC,
              formatted_address TEXT,        -- (保留) 列表顯示
              created_at TIMESTAMPTZ,
              updated_at TIMESTAMPTZ,
              favorites_count BIGINT,
              favorited_at TIMESTAMPTZ,
              approximate_location JSONB,  -- (新增) 地圖顯示
              "user" JSON
...
-- (SELECT ST_AsText(coordinates) FROM public.locations WHERE user_id = v_current_uid AND is_primary = true LIMIT 1) AS debug_user_location_wkb,
...
FROM public.items i
         ...
         LEFT JOIN public.locations l ON i.user_id = l.user_id AND l.is_primary = i.use_primary_location -- (使用 v7.0 邏輯)
