...
BEGIN
    ...
    IF v_current_user_id IS NULL THEN
        RAISE EXCEPTION '使用者未登入，無法獲取追蹤列表';
    END IF;
    ...
    FROM
        public.following f
    INNER JOIN
        public.users u ON f.following_id = u.id
    WHERE
        f.follower_id = v_current_user_id  -- 我追蹤的人（follower_id 是我）
        AND (
            p_search = ''
...
-- 設定函式註解
COMMENT ON FUNCTION public.get_my_following IS '獲取我追蹤的人列表，支援分頁、排序和搜尋功能';
