-- ============================================================================
-- Messaging System v2 - RPC Functions Final Fix
-- ============================================================================
-- 描述: 完整修正欄位名稱衝突問題
-- 版本: 2.0.3
-- 日期: 2024-01-15
-- 修正策略:
--   1. 為 conversation_items_v2 的唯一約束命名
--   2. 在 ON CONFLICT 中使用約束名稱
--   3. RETURN QUERY 直接返回變數,讓 PostgreSQL 自動對應
-- ============================================================================
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE conname = 'uq_conversation_items_v2_conv_item'
    ) THEN
        -- 先刪除舊的未命名約束
        ALTER TABLE public.conversation_items_v2
        DROP CONSTRAINT IF EXISTS conversation_items_v2_conversation_id_item_id_key;

        -- 建立新的命名約束
        ALTER TABLE public.conversation_items_v2
        ADD CONSTRAINT uq_conversation_items_v2_conv_item
        UNIQUE (conversation_id, item_id);

        RAISE NOTICE '已建立命名唯一約束: uq_conversation_items_v2_conv_item';
    END IF;
END $$;
...
BEGIN
    ...
    IF v_conversation_id IS NULL THEN
        ...
        IF p_initial_item_id IS NOT NULL THEN
            ...
            ON CONFLICT ON CONSTRAINT uq_conversation_items_v2_conv_item DO NOTHING;
        END IF;
    ELSE
        IF p_initial_item_id IS NOT NULL THEN
            INSERT INTO public.conversation_items_v2 (
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '✅ 修正內容:';
    RAISE NOTICE '  1. 為 conversation_items_v2 建立命名唯一約束';
    RAISE NOTICE '  2. 使用 ON CONSTRAINT 代替欄位名稱避免歧義';
    RAISE NOTICE '  3. RETURN QUERY 直接返回變數,PostgreSQL 自動對應欄位';
    RAISE NOTICE '  4. 解決 "column reference is ambiguous" 錯誤';
    RAISE NOTICE '';
    RAISE NOTICE '✅ 所有函數已授予 authenticated 角色執行權限';
    RAISE NOTICE '========================================';
    RAISE NOTICE '現在可以在前端測試「聯繫賣家」功能了!';
    RAISE NOTICE '========================================';
END $$;
