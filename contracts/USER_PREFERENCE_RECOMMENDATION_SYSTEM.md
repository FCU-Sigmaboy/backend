...
### v2.0.0 (2025-11-22) - 重大更新
根據專案最新狀態進行的重大更新：

**架構更新：**
- ✅ 修復位置系統變更（items 表不再使用 location_id，改用 user 的主要位置）
- ✅ 更新所有推薦函數以適應新的位置關聯機制

**新功能整合：**
- ✅ 整合評價系統（ratings）- 賣家信譽作為推薦因素
- ✅ 整合交易系統（transactions）- 成功交易記錄影響推薦
- ✅ 整合追蹤系統（following）- 社交推薦維度
- ✅ 新增每日簽到系統考量（用戶活躍度）

**新增推薦算法：**
- ✅ `get_following_items()` - 基於追蹤的社交推薦
- ✅ `get_high_rated_seller_items()` - 基於賣家評價的推薦

**推薦算法優化：**
- 重新平衡推薦分數權重（類別 35%、價格 18%、狀態 12%、距離 10%、賣家評分 10%、新鮮度 8%、交易記錄 5%、社交關係 7%）
- 加入加權互動計算（收藏 x3、聯繫賣家 x2、一般 x1）
- 推薦原因新增 'following_seller' 和 'high_rated_seller'

### v1.0.0 (2025-10-29) - 初始版本
- 完整的推薦系統架構設計
- 基礎資料庫表和 RPC 函數
- Edge Functions API 實作
- 多種推薦算法實現
