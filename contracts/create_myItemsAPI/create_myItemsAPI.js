 ...
 * - itemData.carbon_value (Number) - 可選，預設為 0
 * - itemData.image_urls (Array<String>) - 可選，預設為空陣列 (已上傳到 Storage)
 * - itemData.tags (Array<String>) - 可選，預設為空陣列 (['#Tag1', '#Tag2'])
 * @returns {Promise<object>} - 回傳新建的 item (e.g., { id: 123, title: '...' })
 */
export async function createItem(itemData) {

    // 1. 準備 RPC 參數 (欄位名需與 SQL 函式參數完全對應)
    // 注意: carbon_value, image_urls, tags 為可選參數，若未提供則傳遞 undefined
    const rpcParams = {
        ...
        p_carbon_value: itemData.carbon_value,  // 可選: 後端會用 COALESCE 處理
        p_image_urls: itemData.image_urls,      // 可選: 後端會用 COALESCE 處理
        p_tags: itemData.tags                   // 可選: 後端會用 COALESCE 處理
    };
